...
class CouponProvider {
    ...
    func fetchAllCoupons(completion: @escaping CouponResponse) {
        ...
        fetchAllCoupons(request: STCouponRequest.allCoupons(token: token), completion: completion)
    }
    func fetchInvalidCoupons(completion: @escaping CouponResponse) {
        guard let token = KeyChainManager.shared.token else {
            return completion(.failure(STYLiSHSignInError.noToken))
        }
        fetchInvalidCoupons(request: STCouponRequest.invalidCoupons(token: token), completion: completion)
    }
    ...
    // MARK: - Private method
    private func fetchAllCoupons(request: STCouponRequest, completion: @escaping CouponResponse) {
    ...
    private func fetchInvalidCoupons(request: STCouponRequest, completion: @escaping CouponResponse) {
